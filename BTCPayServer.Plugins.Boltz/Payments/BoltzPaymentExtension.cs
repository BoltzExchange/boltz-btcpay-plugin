...
public class BoltzPaymentModelExtension : IPaymentModelExtension
{
    ...
    public BoltzPaymentModelExtension(
    ...
    public void ModifyPaymentModel(PaymentModelContext context)
    {
        ...
        if (context.Model.Activated)
        {
            ...
            if (_bech32Prefix is not null &&
                context.Model.BtcAddress.StartsWith(_bech32Prefix, StringComparison.OrdinalIgnoreCase))
            {
                context.Model.InvoiceBitcoinUrlQR = context.Model.InvoiceBitcoinUrlQR.Replace(
                    $"{_Network.NBitcoinNetwork.UriScheme}:{context.Model.BtcAddress}",
                    $"{_Network.NBitcoinNetwork.UriScheme}:{context.Model.BtcAddress.ToUpperInvariant()}",
