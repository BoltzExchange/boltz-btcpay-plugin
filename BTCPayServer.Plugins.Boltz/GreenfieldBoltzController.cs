...
) : ControllerBase
{
    ...
    public async Task<IActionResult> EnableSetup(string storeId, [FromBody] BoltzSetupRequest request)
    {
        ...
        {
            return this.CreateAPIError("boltz-unavailable", e.Message);
        }
        catch (Exception e)
        {
    ...
    public async Task<IActionResult> CreateWallet(string storeId, [FromBody] CreateBoltzWalletRequest request)
    {
        ...
        try
        {
            ...
            if (client == null)
            {
                return this.CreateAPIError("boltz-unavailable", "Boltz daemon is not available. Please ensure Boltz is properly configured.");
            }
